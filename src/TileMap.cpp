...
void TileMap::loadMap(const int* tiles, unsigned int mapWidth, unsigned int mapHeight, sf::View view)
{
  ...
  sf::Vector2f chunk = view.getSize() * 1.0f;
...
{
  float chunkLimit_x = m_chunkSize.x * m_tileSize.x / 2;
  float chunkLimit_y = m_chunkSize.y * m_tileSize.y / 2;
  ...
  std::cout << "c" << m_center.x << " " << m_center.y << std::endl;

  // flash(sf::Vector2f(t_x / m_tileSize.x, t_y / m_tileSize.y));
}

void TileMap::flash(sf::Vector2f t)
{
  // Populate the vertex array, with one quad per tile
  for (unsigned int i = t.x; i < m_chunkSize.x + t.x; i++)
  {
    for (unsigned int j = t.y; j < m_chunkSize.y + t.y; j++)
    {
      // Get the current tile number
      int tileNumber = m_tiles[i + j * m_chunkSize.x];
      ...
      sf::Vertex* quad = &m_vertices[(i + j * m_chunkSize.x) * 4];
